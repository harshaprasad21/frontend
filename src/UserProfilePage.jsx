...
const UserProfilePage = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://api.cloudchampion.online/get-upload-image?userId=${userData?._id}`,
          {
            headers: {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const response = await axios.post(
        `https://api.cloudchampion.online/upload-s3-image?userId=${userData?._id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleLogout=async(e)=>{
    e.preventDefault();
    const logoutRes=await axios.post('https://api.cloudchampion.online/logout',{headers:{'Content-Type': 'application/json'},withCredentials:true});
    dispatch(doLogout());
